...
Start:
 ...
 ;PORTB
   ;Bank 3
    BSF STATUS,5 ;Bit Set File Address -> To STATUS Register -> ((STATUS) and 07Fh), 5 corresponds to bit 5
    BSF STATUS,6 ;Bit Set File Address -> To STATUS Register -> ((STATUS) and 07Fh), 5 corresponds to bit 5
...
COUNT EQU 0x22
LED_STATE EQU 0x23
STORED_KEYPAD EQU 0x24
RECORD_COUNT EQU 0x25
RECORD_MODE EQU 0x26
TMR2COUNT EQU 0x27 ; 0..49
ONESEC_FLAG EQU 0x28 ; bit0 used as latch
;---------------------------------------------------------------------
WRITE:
    BANKSEL EEDATA
    MOVWF EEDATA
    ...
    MOVLW 0x55 ; MOVWF EECON2 ;- MOVLW 0xAA ;-- MOVWF EECON2 ;--- BSF EECON1,1 ;NECESSARY CODE FROM DATASHEET PG.113
    ...
    WAIT:
    BANKSEL PIR2
    BTFSS PIR2,4 ;EE WRITE COMPLETED FLAG
    GOTO WAIT

    BCF PIR2,4 ;CLEARS FLAG, READY FOR NEW DATA
...
MAIN:
;RECORD_MODE BIT,0 - INDICATES RECORDING STATUS
;RECORD_MODE BIT,1 - INDICATES PLAYBACK STATUS
;RECORD_FLAG(PORTA,1) - BUTTON FLAG WILL EXECUTE RECORD
;PLAYBACK_RECORD(PORTA,1) - BUTTON FLAG WILL EXECUTE PLAYBACK
;WAIT_FOR_RECORD - SUBROUTINE WHICH WAITS FOR A BUTTON TO BE PRESSED
    BTFSC RECORD_MODE,1 ;IF ON, SET BY PLAYBACK BUTTON, WILL LOOP TO MAIN
    GOTO MAIN

    BTFSC PORTA,0
    CALL RECORD_FLAG ;BUTTON WHICH SETS THE RECORDING FLAG

    BTFSC PORTA,1
    CALL PLAYBACK_RECORD ;BUTTON WHICH SETS THE PLAYBACK FLAG

    BTFSC RECORD_MODE,0 ;IF ON, SET BY RECORD BUTTON, WLIL SKIP START_RECORD
    CALL START_RECORD
...
INTERRUPT_HANDLER:
    ...
    INCF TMR2COUNT,1 ;+1 every 20 ms
    MOVLW 0x32 ;50 * 20 ms ? 1 s
    SUBWF TMR2COUNT,0
    BTFSS STATUS,2
    GOTO Clear_IF_TMR2
    CLRF TMR2COUNT ;roll over each second
    BSF ONESEC_FLAG, 0 ;set "one-second happened"
Clear_IF_TMR2:
    BCF PIR1,1 ;clear ((PIR1) and 07Fh), 1 (bit1)
RETURN_FROM_ISR:
...
;ROW 4(BOTTOM)
    BSF PORTB,3 ;Sets ((PORTB) and 07Fh), 3 High
    BTFSC PORTB,7 ;Checks column 4, ((PORTB) and 07Fh), 7
    MOVLW 0x46
    BTFSC PORTB,6 ;Checks column 3, ((PORTB) and 07Fh), 6
    MOVLW 0x45
    BTFSC PORTB,5 ;Checks column 2, ((PORTB) and 07Fh), 5
    MOVLW 0x44
    BTFSC PORTB,4 ;Checks column 1, ((PORTB) and 07Fh), 4
    MOVLW 0x43
    MOVWF STORED_KEYPAD ;STORES KEYPAD PRESS IN A WARM SPOT
    CALL BUTTON_RELEASED
    MOVF STORED_KEYPAD,0
    BCF PORTB,3

;ROW 3
    BSF PORTB,2 ;Sets ((PORTB) and 07Fh), 2 High
    BTFSC PORTB,7 ;Checks column 4, ((PORTB) and 07Fh), 7
    MOVLW 0x42
    BTFSC PORTB,6 ;Checks column 3, ((PORTB) and 07Fh), 6
    MOVLW 0x41
...
;ROW 2
    BSF PORTB,1 ;Sets ((PORTB) and 07Fh), 1 High
    BTFSC PORTB,7 ;Checks column 4, ((PORTB) and 07Fh), 7
    MOVLW 0x37 ;'7'
    BTFSC PORTB,6 ;Checks column 3, ((PORTB) and 07Fh), 6
    MOVLW 0x36 ;'6'
    ...
    BCF PORTB,1
;ROW 1
    BSF PORTB,0 ;Sets ((PORTB) and 07Fh), 0 High
    BTFSC PORTB,7 ;Checks column 4, ((PORTB) and 07Fh), 7
    MOVLW 0x33 ;'3'
    BTFSC PORTB,6 ;Checks column 3, ((PORTB) and 07Fh), 6
    MOVLW 0x32 ;'2'
    ...
    INCF RECORD_COUNT
    MOVF RECORD_COUNT,0 ;MOVES RECORD COUNT(0-10) TO W
    BANKSEL EEADR
    MOVWF EEADR ;MOVES W TO EEADR, IS ASSIGNED AN ADDRESS
       ;FROM 0-10
...
BUTTON_RELEASED:
    ...
    BTFSS STATUS,2 ;CHECK ((STATUS) and 07Fh), 2 FLAG, IF 0 CHECK AGAIN
